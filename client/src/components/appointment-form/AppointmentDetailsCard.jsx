...
export function AppointmentDetailsCard({
  ...
  const showSlotPicker = hasConfiguredAvailability;

  function isDateDisabled(dateString) {
    if (!dateString || !hasConfiguredAvailability || !availableDays?.size) return false;

    const [y, m, d] = dateString.split("-").map(Number);
    const dayName = new Date(y, m - 1, d, 12).toLocaleDateString("en-US", {
      weekday: "long",
    });
    ...
    <Card className="rounded-2xl p-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          />
          {hasConfiguredAvailability ? <p className="text-xs text-muted-foreground">Select a date that matches doctor availability.</p> : null}
        </div>
        <div className="space-y-2">
          <Label>Time</Label>
          {showSlotPicker ? (
            slotsLoading ? (
              ...
              <p className="text-sm text-destructive">
                {form.date ? "No available time slots for this date." : "Select a date first."}
              </p>
            ) : (
              <select
                value={form.time}
