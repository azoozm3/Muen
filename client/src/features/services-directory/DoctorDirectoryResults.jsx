...
export default function DoctorDirectoryResults({ doctors, isLoading, isError, error, hasActiveFilters, clearFilters, onRetry, onBook }) {
  if (isLoading) {
  ...
  if (isError) {
    return (
      <Card className="p-8 text-center">
        ...
        <h3 className="text-lg font-semibold">Doctors could not load</h3>
        <p className="mt-1 text-sm text-muted-foreground">{error?.message || "Please check the server and database connection."}</p>
        <Button variant="outline" size="sm" onClick={onRetry} className="mt-4">Try Again</Button>
      </Card>
    );
  }
