...
export function appointmentErrorResponse(res, error, fallbackMessage) {
  ...
  if (message === "DOCTOR_UNAVAILABLE") return res.status(400).json({ message: "This doctor is currently unavailable for booking" });
  if (message === "SLOT_NOT_AVAILABLE") return res.status(400).json({ message: "This time slot is not available for this doctor" });
  if (message === "SLOT_ALREADY_BOOKED") return res.status(409).json({ message: "This time slot was already booked" });
